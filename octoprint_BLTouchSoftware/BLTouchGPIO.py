...
class BLTouchGPIO:
	...
	def __init__(self):
		...
		GPIO.add_event_detect(self.GPIO_BLTouch_Zmin, GPIO.RISING, callback=self.callback_bltouch_zmin, bouncetime=75)

		# callback_bltouch_zmin when the zswitch is trigger
		GPIO.setup(self.GPIO_Switch_Zmin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
		GPIO.add_event_detect(self.GPIO_Switch_Zmin, GPIO.FALLING, callback=self.callback_switch_zmin)
	...
	def callback_bltouch_zmin(self, channel):
			self.trigger = True
			self.printlog("BLTOUCH TRIGGER! channel=%s, state=%s" % (channel, GPIO.input(channel)))
